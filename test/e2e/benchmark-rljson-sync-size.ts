...
async function benchmark(): Promise<void> {
  ...
  try {
    ...
    console.log('─'.repeat(70));
    console.log(
      `${colors.cyan}${colors.bold}Step 1: Preparing Sample${colors.reset}`,
    );
    console.log('─'.repeat(70));
    console.log(
      `${colors.blue}ℹ${colors.reset} Creating ${formatNumber(SAMPLE_SIZE)} document sample...`,
    );
    ...
    console.log(
      `${colors.green}✓${colors.reset} Sample created: ${formatNumber(count)} documents (${formatBytes(stats.size)})`,
    );
    console.log('');
    ...
    console.log('─'.repeat(70));
    console.log(
      `${colors.cyan}${colors.bold}Step 2: RLJSON Extraction${colors.reset}`,
    );
    console.log('─'.repeat(70));
    ...
    console.log(
      `${colors.blue}ℹ${colors.reset} Extracting documents to RLJSON tree + blob storage...`,
    );
    ...
    console.log(
      `${colors.green}✓${colors.reset} Extracted in ${formatTime(extractTime)}`,
    );
    ...
    console.log('─'.repeat(70));
    console.log(
      `${colors.cyan}${colors.bold}Step 3: RLJSON Sync Payload Size${colors.reset}`,
    );
    console.log('─'.repeat(70));

    // Convert tree to sync payload (tree structure only, no blob content)
    console.log(
      `${colors.blue}ℹ${colors.reset} Serializing tree structure for sync...`,
    );
    ...
    console.log('─'.repeat(70));
    console.log(
      `${colors.cyan}${colors.bold}Step 4: Blob Retrieval Performance${colors.reset}`,
    );
    console.log('─'.repeat(70));

    console.log(
      `${colors.blue}ℹ${colors.reset} Measuring blob retrieval time...`,
    );
    ...
    if (retrievedCount > 0) {
      console.log(
        `${colors.green}✓${colors.reset} Retrieved ${formatNumber(retrievedCount)} blobs in ${formatTime(retrieveTime)}`,
      );
      console.log(
        `  Throughput: ${formatNumber(Math.round(retrievedCount / (retrieveTime / 1000)))} blobs/sec`,
      );
      console.log(
        `  Time per blob: ${(retrieveTime / retrievedCount).toFixed(3)}ms`,
      );
      console.log(
        `  Average blob size: ${formatBytes(totalBlobSize / retrievedCount)}`,
      );
      console.log('');
      ...
      console.log(
        `  ${colors.magenta}Estimated time to retrieve all ${formatNumber(blobIds.length)} blobs:${colors.reset}`,
      );
      console.log(`    ${formatTime(fullRetrieveTime)}`);
    } else {
      console.log(
        `${colors.yellow}⚠${colors.reset} No blobs retrieved (blob storage issue)`,
      );
    }
    console.log('');
    ...
    console.log('─'.repeat(70));
    console.log(
      `${colors.cyan}${colors.bold}Step 5: Extrapolation to 552k Documents${colors.reset}`,
    );
    console.log('─'.repeat(70));
    ...
    const fullRetrieveTime =
      retrievedCount > 0 ? (retrieveTime / retrievedCount) * fullCount : 0;

    console.log(
      `  ${colors.bold}Full Dataset (${formatNumber(fullCount)} documents):${colors.reset}`,
    );
    console.log('');
    console.log(`  Extraction (MongoDB → RLJSON):`);
    console.log(`    Time: ${formatTime(fullExtractTime)}`);
    console.log(
      `    Throughput: ${formatNumber(Math.round(count / (extractTime / 1000)))} docs/sec`,
    );
    console.log('');
    ...
    console.log(
      `    Per document: ${formatBytes(fullPayloadSize / fullCount)}`,
    );
    console.log('');
    ...
    console.log(
      `    Transfer payload: ~${formatTime((fullPayloadSize * 0.4) / (10 * 1024 * 1024))} (@ 10MB/s network)`,
    );
    if (retrievedCount > 0) {
    ...
    if (retrievedCount > 0) {
      console.log(
        `    Blob retrieval: ${formatNumber(retrievedCount)} in ${formatTime(retrieveTime)}`,
      );
    }
    console.log('');
    console.log(`  ${colors.bold}For 552k documents:${colors.reset}`);
    console.log(
      `    ${colors.cyan}→${colors.reset} RLJSON tree payload: ${formatBytes(fullPayloadSize)} (${formatBytes(fullPayloadSize * 0.4)} compressed)`,
    );
    console.log(
      `    ${colors.cyan}→${colors.reset} Full extraction: ~${formatTime(fullExtractTime)}`,
    );
    if (retrievedCount > 0) {
      console.log(
        `    ${colors.cyan}→${colors.reset} Full blob retrieval: ~${formatTime(fullRetrieveTime)}`,
      );
    }
    console.log('═'.repeat(70) + '\n');
