...
async function test() {
  ...
  try {
    ...
    for (const col of tableCfg.columns) {
      console.log(
        `  ${col.key.padEnd(15)} ${col.type.padEnd(10)} "${col.titleLong}" / "${col.titleShort}"`,
      );
    }
    ...
    console.log(
      `${colors.green}✓${colors.reset} Document converted and hashed\n`,
    );

    // Test 3: Collection Conversion
    console.log(
      `${colors.cyan}Test 3: Collection Conversion to ComponentsTable${colors.reset}`,
    );
    console.log('─'.repeat(50));
    ...
    for (const row of componentsTable._data) {
      console.log(
        `    - ${row._id}: ${row.name} (hash: ${row._hash.substring(0, 8)}...)`,
      );
    }

    console.log(
      `${colors.green}✓${colors.reset} Collection converted to ComponentsTable\n`,
    );
    ...
    for (const row of componentsTable._data) {
      if (!row._hash) {
        console.log(
          `${colors.yellow}⚠${colors.reset} Row ${row._id} has no hash`,
        );
        allValid = false;
      }
    }
    ...
    if (componentsTable._hash) {
      console.log(
        `${colors.green}✓${colors.reset} ComponentsTable properly hashed`,
      );
    } else {
      console.log(
        `${colors.yellow}⚠${colors.reset} ComponentsTable has no hash`,
      );
      allValid = false;
    }
    ...
    if (componentsTable._tableCfg === tableCfg._hash) {
      ...
      console.log(
        `${colors.yellow}⚠${colors.reset} TableCfg reference mismatch`,
      );
      allValid = false;
    }
