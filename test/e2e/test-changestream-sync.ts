...
async function testChangeStreamMonitoring(): Promise<void> {
  ...
  try {
    ...
    const syncOpsA = await dbA.collection<SyncOp>('sync_ops').findOne({
      origin: 'nodeA',
    ...
    const updateOp = await dbA.collection<SyncOp>('sync_ops').findOne({
      origin: 'nodeA',
    ...
    const deleteOp = await dbA.collection<SyncOp>('sync_ops').findOne({
      origin: 'nodeA',
    ...
    const localState = await dbA
      .collection<SyncLocal>('sync_local')
      .findOne({ _id: 'local' });
...
async function testBidirectionalSync(): Promise<void> {
...
async function testHashChainIntegrity(): Promise<void> {
  ...
  try {
    ...
    const syncOps = db.collection<SyncOp>('sync_ops');
