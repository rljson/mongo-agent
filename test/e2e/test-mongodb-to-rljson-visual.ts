...
async function main() {
  ...
  try {
    ...
    console.log('📊 Data Types: ObjectId, String, Number, Boolean, Array, Object');
    console.log('🔍 MongoDB-specific features: _id as ObjectId, nested objects');
    ...
    console.log(`  _tableCfg: "${componentsTable._tableCfg}",  ← References TableCfg by hash`);
    ...
    const tableCfgsBlobProps = await bs.setBlob(Buffer.from(tableCfgsSerialized, 'utf-8'));
    ...
    console.log(`   ✓ Retrieved ComponentsTable with ${retrievedComponentsTable._data.length} rows`);
    console.log(`   ✓ TableCfg reference: ${retrievedComponentsTable._tableCfg}`);
    ...
    console.log(`   ✓ Retrieved TablesCfgTable with ${retrievedTablesCfgsTable._data.length} schema(s)`);
    ...
    for (let i = 0; i < retrievedComponentsTable._data.length; i++) {
      ...
      for (const col of matchingTableCfg.columns) {
        ...
        console.log(`   ${col.titleLong.padEnd(20)} (${col.type.padEnd(10)}): ${displayValue}`);
      }
      console.log('');
    }
    ...
    console.log(`   TablesCfgTable Blob: ${rootMeta.tableCfgsTableBlobId?.substring(0, 20)}...`);
    ...
    for (const [hash, node] of tree.trees) {
      ...
      if (meta?.type === 'collection') {
        ...
        console.log(`     TableCfg hash: ${meta.tableCfgHash?.substring(0, 20)}...`);
        console.log(`     ComponentsTable blob: ${meta.componentsBlobId?.substring(0, 20)}...`);
        console.log(`     Document count: ${meta.docCount}`);
        
        // Retrieve and show snippet
        if (meta.componentsBlobId) {
          const compTable = await scanner.getComponentsTable(meta.componentsBlobId);
          console.log(`     First row keys: ${Object.keys(compTable._data[0]).join(', ')}`);
        }
      }
    }
    ...
    if (rootMeta.tableCfgsTableBlobId) {
      const tableCfgsTableFull = await scanner.loadTablesCfgTable(rootMeta.tableCfgsTableBlobId);
      console.log(`   Total schemas: ${tableCfgsTableFull._data.length}`);
      for (const cfg of tableCfgsTableFull._data) {
        console.log(`     - ${cfg.key} (${cfg.columns.length} columns, hash: ${cfg._hash?.substring(0, 20)}...)`);
      }
    }
