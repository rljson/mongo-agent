...
async function main() {
  ...
  try {
    ...
    console.log(
      '✅ Created test collections: users (3 docs), products (3 docs)\n',
    );
    ...
    console.log(
      `  ✓ Scanned database, root hash: ${tree.rootHash.substring(0, 16)}...`,
    );
    ...
    console.log(
      `  ✓ TablesCfgTable blob ID: ${tableCfgsTableBlobId.substring(0, 16)}...`,
    );

    // Test 2: Load TablesCfgTable from blob
    console.log('\n📥 Test 2: Load TablesCfgTable from blob storage');
    const tableCfgsTable =
      await scanner.loadTablesCfgTable(tableCfgsTableBlobId);

    console.log(
      `  ✓ Loaded TablesCfgTable with ${tableCfgsTable._data.length} schemas`,
    );
    ...
    for (const key of expectedKeys) {
      ...
      console.log(
        `  ✓ Found TableCfg for '${key}' (${tableCfg.columns.length} columns)`,
      );
    ...
    const syncOpsFromTable = tableCfgsTable._data.find(
      (cfg) => cfg.key === 'sync_ops',
    );
    ...
    if (syncOpsFromTable._hash !== SYNC_OPS_TABLE_CFG._hash) {
      throw new Error(
        `❌ sync_ops hash mismatch: ${syncOpsFromTable._hash} !== ${SYNC_OPS_TABLE_CFG._hash}`,
      );
    }

    console.log(
      `  ✓ sync_ops TableCfg matches expected hash: ${syncOpsFromTable._hash?.substring(0, 16)}...`,
    );
    ...
    const usersTableCfg = tableCfgsTable._data.find(
      (cfg) => cfg.key === 'users',
    );
    ...
    for (const colKey of expectedUserColumns) {
      if (!actualColumns.includes(colKey)) {
        throw new Error(
          `❌ Expected column '${colKey}' not found in users schema`,
        );
      }
    ...
    if (retrievedCfg.key !== 'users') {
      throw new Error(
        `❌ Retrieved wrong TableCfg: ${retrievedCfg.key} !== users`,
      );
    }
    ...
    for (const collTree of collectionTrees) {
      ...
      const referencedCfg = scanner.getTableCfgByHash(
        tableCfgsTable,
        tableCfgHash,
      );
      ...
      if (!referencedCfg) {
        throw new Error(
          `❌ ComponentsTable for '${collName}' references unknown TableCfg hash: ${tableCfgHash}`,
        );
      }
      ...
      if (componentsTable._tableCfg !== tableCfgHash) {
        throw new Error(
          `❌ ComponentsTable._tableCfg mismatch for ${collName}`,
        );
      }

      console.log(
        `  ✓ Collection '${collName}' ComponentsTable references correct schema`,
      );
      console.log(
        `    TableCfg: ${referencedCfg.key} (${referencedCfg.columns.length} columns)`,
      );
    ...
    if (reloadedTable._data.length !== tableCfgsTable._data.length) {
      throw new Error(
        '❌ Reloaded TablesCfgTable has different number of schemas',
      );
    }
    ...
    for (const tableCfg of tableCfgsTable._data) {
      ...
      if (!tableCfg.key || typeof tableCfg.key !== 'string') {
        throw new Error(
          `❌ TableCfg missing or invalid 'key': ${tableCfg.key}`,
        );
      }

      if (!tableCfg.type || typeof tableCfg.type !== 'string') {
        throw new Error(
          `❌ TableCfg '${tableCfg.key}' missing or invalid 'type'`,
        );
      }

      if (!Array.isArray(tableCfg.columns)) {
        throw new Error(
          `❌ TableCfg '${tableCfg.key}' missing or invalid 'columns'`,
        );
      }

      if (typeof tableCfg.isHead !== 'boolean') {
        throw new Error(
          `❌ TableCfg '${tableCfg.key}' missing or invalid 'isHead'`,
        );
      }

      if (typeof tableCfg.isRoot !== 'boolean') {
        throw new Error(
          `❌ TableCfg '${tableCfg.key}' missing or invalid 'isRoot'`,
        );
      }

      if (typeof tableCfg.isShared !== 'boolean') {
        throw new Error(
          `❌ TableCfg '${tableCfg.key}' missing or invalid 'isShared'`,
        );
      }
      ...
      if (!tableCfg._hash || typeof tableCfg._hash !== 'string') {
        throw new Error(
          `❌ TableCfg '${tableCfg.key}' missing or invalid '_hash'`,
        );
      }
    ...
    console.log(`\nSummary:`);
    console.log(
      `  • Total schemas in TablesCfgTable: ${tableCfgsTable._data.length}`,
    );
    console.log(
      `  • Collection schemas: ${tableCfgsTable._data.filter((c) => c.key !== 'sync_ops').length}`,
    );
