...
/**
 * Real-Time Blob-Based Sync Test with MongoDB Change Streams
 *
 * This test demonstrates REAL-TIME synchronization using:
 * 1. MongoDB Change Streams to detect changes
 * 2. Automatic blob storage of changed documents
 * 3. Real-time sync to second agent
 * 4. Verification that changes propagate correctly
 *
 * Uses simplified blob approach (no schema required) for maximum compatibility.
 */
...
async function main() {
  ...
  try {
    header('REAL-TIME BLOB-BASED SYNC TEST WITH CHANGE STREAMS');
    ...
    changeStream.on('change', async (change) => {
      ...
      const ns = `${change.ns.db}.${collName}`;
      ...
      // Trigger blob-based sync (simplified - no schema required)
      try {
        let docsApplied = 0;
        ...
        if (
          ...
          info(`  → Fetching and storing document as blob...`);

          // Get the full document
          const doc = await mongoA.collection(collName).findOne({ _id: docId });

          if (doc) {
            // Store as blob
            const docJson = JSON.stringify(doc);
            const buffer = Buffer.from(docJson, 'utf-8');
            const blobProps = await bsA.setBlob(buffer);

            // Also store in target blob storage
            await bsB.setBlob(buffer);

            info(`  → Syncing to Agent B (blob: ${blobProps.blobId})...`);
          ...
          info(`  → Syncing deletion to Agent B...`);

          // Delete from target
          await mongoB.collection(collName).deleteOne({ _id: docId });

          docsApplied = 1;
        }

        syncCount++;
        success(`  ✓ Sync #${syncCount} completed (${docsApplied} docs)`);
      } catch (err) {
