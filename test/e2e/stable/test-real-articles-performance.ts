...
async function main() {
  ...
  try {
    ...
    if (existingCache > 0) {
      ...
      console.log(
        `✓ Current state hash: ${initialState.dbRoot.slice(0, 16)}...`,
      );
    } else {
      ...
      console.log(
        `✓ Full state hash computed: ${initialState.dbRoot.slice(0, 16)}...`,
      );
      console.log(
        `⏱️  Time: ${fullHashDuration}ms (${(fullHashDuration / 1000).toFixed(2)}s)`,
      );
    ...
    // ═══════════════════════════════════════════════════════════════════════
    console.log(
      '📡 PART 3: CRUD Operations on Real Articles (Real-Life Scenario)\n',
    );
      ...
      await collection.updateOne(
        { _id: articleToUpdate._id },
        {
          $set: {
            ...
            testField: 'Updated by performance test',
          },
        },
      );
    ...
    console.log('\nOperation 2: Update another article');
    const articleToUpdate2 = await collection.findOne({
      _id: { $ne: articleToUpdate?._id },
    });
    if (articleToUpdate2) {
      await collection.updateOne(
        { _id: articleToUpdate2._id },
        {
          $set: {
            lastModified: new Date(),
            testField2: 'Second update by performance test',
          },
        },
      );
    ...
    const excludeIds = [articleToUpdate?._id, articleToUpdate2?._id].filter(
    ...
    if (articleToUpdate3) {
      await collection.updateOne(
        { _id: articleToUpdate3._id },
        {
          $set: {
            lastModified: new Date(),
            testField3: 'Third update by performance test',
          },
        },
      );
    ...
    console.log(
      `📊 Dirty partitions: ${dirtyCount} out of ${cachedPartitions} (~${((dirtyCount / cachedPartitions) * 100).toFixed(1)}%)`,
    );
    console.log(
      `🔥 Full rescan flag: ${dirtyStatus.full ? 'YES (will do full scan)' : 'NO (incremental mode OK)'}\n`,
    );
    ...
    console.log(
      `✓ Incremental hash computed: ${incrementalState.dbRoot.slice(0, 16)}...`,
    );
    console.log(
      `⏱️  Time: ${incrementalHashDuration}ms (${(incrementalHashDuration / 1000).toFixed(2)}s)`,
    );
    ...
    if (fullHashDuration > 0) {
      ...
      console.log(
        `   Cache hit rate:   ${(((cachedPartitions - dirtyCount) / cachedPartitions) * 100).toFixed(1)}%`,
      );
    ...
    if (existingCountB > 0) {
      console.log(
        `✓ Node B already has ${existingCountB.toLocaleString()} documents`,
      );
      ...
      for await (const doc of cursor) {
        ...
        if (batch.length >= batchSize) {
          ...
          if (copiedCount % 50000 === 0) {
            console.log(
              `  Copied ${copiedCount.toLocaleString()} documents...`,
            );
          }
        }
      }
      ...
      console.log(
        `✓ Copied ${copiedCount.toLocaleString()} documents in ${copyDuration}ms (${(copyDuration / 1000).toFixed(2)}s)`,
      );
      console.log(
        `  Throughput: ${(copiedCount / (copyDuration / 1000)).toFixed(0)} docs/sec\n`,
      );
    }
    ...
    if (existingCountC > 0) {
      console.log(
        `✓ Node C already has ${existingCountC.toLocaleString()} documents`,
      );
      ...
      for await (const doc of cursorBackfill) {
        ...
        if (batchBackfill.length >= batchSize) {
          ...
          if (backfilledCount % 50000 === 0) {
            console.log(
              `  Backfilled ${backfilledCount.toLocaleString()} documents...`,
            );
          }
        }
      }
      ...
      console.log(
        `  Throughput: ${(backfilledCount / (backfillDuration / 1000)).toFixed(0)} docs/sec`,
      );
    }
    ...
    console.log('');
    console.log(
      `Dataset:              ${finalDocCount.toLocaleString()} documents`,
    );
    console.log(`Partitions:           ${cachedPartitions} (50k docs each)`);
    console.log(
      `Dirty partitions:     ${dirtyCount} (~${((dirtyCount / cachedPartitions) * 100).toFixed(1)}%)`,
    );

    if (fullHashDuration > 0) {
      console.log(
        `Full state hash:      ${fullHashDuration}ms (${(fullHashDuration / 1000).toFixed(2)}s)`,
      );
      console.log(
        `Incremental hash:     ${incrementalHashDuration}ms (${(incrementalHashDuration / 1000).toFixed(2)}s)`,
      );
      console.log(`Speedup:              ${speedup}x faster`);
    } else {
      ...
      console.log(
        `Cache benefit:        Only ${dirtyCount}/${cachedPartitions} partitions recomputed!`,
      );
    }

    if (copyDuration > 0) {
      console.log(
        `Initial sync:         ${copyDuration}ms (${(copyDuration / 1000).toFixed(2)}s)`,
      );
      console.log(
        `Sync throughput:      ${(copiedCount / (copyDuration / 1000)).toFixed(0)} docs/sec`,
      );
    } else {
    ...
    if (backfillDuration > 0) {
      console.log(
        `Backfill time:        ${backfillDuration}ms (${(backfillDuration / 1000).toFixed(2)}s)`,
      );
      console.log(
        `Backfill throughput:  ${(backfilledCount / (backfillDuration / 1000)).toFixed(0)} docs/sec`,
      );
    } else {
    ...
    console.log(
      '  Real-Life Scenario: Cache reused, only dirty partitions recomputed',
    );
    console.log('═'.repeat(80));
    console.log('');
  } finally {
