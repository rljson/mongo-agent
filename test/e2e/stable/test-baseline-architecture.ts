...
async function main() {
  ...
  try {
    ...
    // Insert a document
    console.log('Inserting document: { name: "Alice", age: 30, role: "engineer" }\n');
    ...
    const user1 = await collection.insertOne({
      ...
      projects: [
        { name: 'Project Alpha', role: 'lead', startDate: new Date('2023-01-01') },
        { name: 'Project Beta', role: 'contributor', startDate: new Date('2023-06-01') }
      ],
      metadata: {
    ...
    const user2 = await collection.insertOne({
      ...
      projects: [
        { name: 'UI Redesign', role: 'lead', startDate: new Date('2024-01-15') }
      ],
      metadata: {
    ...
    const user3 = await collection.insertOne({
      ...
      projects: [
        { name: 'Product Roadmap 2026', role: 'owner', startDate: new Date('2025-12-01') },
        { name: 'Market Analysis', role: 'lead', startDate: new Date('2025-08-01') }
      ],
      metadata: {
    ...
    console.log(`  ✓ Updated Bob: promoted to senior, raised salary, relocated`);
    ...
    await collection.replaceOne(
      { _id: user3.insertedId },
      {
        ...
        department: 'Product',
        joinDate: new Date('2019-03-10'),
        skills: ['Product Management', 'Strategy', 'Leadership', 'Analytics', 'OKRs'],
        address: {
          street: '789 Pine Rd',
          city: 'Seattle',
          state: 'WA',
        ...
        projects: [
          { name: 'Product Roadmap 2026', role: 'owner', startDate: new Date('2025-12-01') },
          { name: 'Market Analysis', role: 'lead', startDate: new Date('2025-08-01') },
          { name: 'Q2 Strategy', role: 'sponsor', startDate: new Date('2026-03-15') }
        ],
        metadata: {
    ...
    const localState = await db.collection('sync_local').findOne({ _id: 'local' } as any);
    console.log('Debug: sync_local seq =', localState ? (localState as any).seq : 'null');
    ...
    if (meta && (meta as any).componentsBlobId) {
      ...
      console.log('Debug: blob =', blob ? 'found' : 'null', blob ? `(${blob.content.length} bytes)` : '');
      ...
      if (blob) {
        ...
        for (let i = 0; i < table._data.length; i++) {
          ...
          const docPreview = op.payload?.fullDocumentBlobId ? '✓ blob' : '(no doc)';
          console.log(`  ${i + 1}. seq=${op.seq} ${op.operationType.padEnd(8)} ${op.ns.coll.padEnd(10)} ${docPreview} chain=${op.chainHash.substring(0, 12)}...`);
        }
        console.log('─'.repeat(80));
        ...
        console.log(`  opHash:                 ${syncOp.opHash.substring(0, 16)}...`);
        console.log(`  chainHash:              ${syncOp.chainHash.substring(0, 16)}...`);
        
        console.log('\nMongoDB Change Stream Metadata (NEW!):');
        console.log(`  changeStreamId:         ${JSON.stringify(syncOp.changeStreamId).substring(0, 80)}...`);
        console.log(`  clusterTime:            ${JSON.stringify(syncOp.clusterTime)}`);
        console.log(`  wallTime:               ${syncOp.wallTime}`);
        
        console.log('\nPayload (RLJSON Blob References):');
        console.log(`  fullDocumentBlobId:     ${syncOp.payload?.fullDocumentBlobId || 'null'}`);
        console.log(`  updateDescriptionBlobId: ${syncOp.payload?.updateDescriptionBlobId || 'null'}`);
        ...
        console.log(`  ✅ prevStateHash:       ${syncOp.prevStateHash || '(computed on demand)'}`);
        console.log(`  ✅ currentStateHash:    ${syncOp.currentStateHash || '(computed on demand)'}`);
        console.log(`  Note: State hashes computed incrementally using Merkle tree`);
        console.log(`  Usage: Client checks "Do I have prevState? → Can apply!"`);
        
        console.log('\nOperation Data:');
        console.log(`  ns.db:                  ${syncOp.ns.db}`);
        console.log(`  ns.coll:                ${syncOp.ns.coll}`);
        console.log(`  docId:                  ${JSON.stringify(syncOp.docId)}`);
...
`);
    ...
    console.log('\n┌─────────────────────────────────┬─────────────────────────────────────┐');
    console.log('│ MongoDB Change Stream           │ RLJSON SyncOpDoc Component          │');
    console.log('├─────────────────────────────────┼─────────────────────────────────────┤');
    console.log('│ _id (resume token)              │ changeStreamId (CAPTURED!)          │');
    console.log('│ clusterTime                     │ clusterTime (CAPTURED!)             │');
    console.log('│ wallTime                        │ wallTime (CAPTURED!)                │');
    console.log('│ operationType                   │ operationType                       │');
    console.log('│ ns.db, ns.coll                  │ ns.db, ns.coll                      │');
    console.log('│ documentKey._id                 │ docId                               │');
    console.log('│ fullDocument (embedded)         │ fullDocumentBlobId (hash ref) ✨    │');
    console.log('│ updateDescription (embedded)    │ updateDescriptionBlobId (hash) ✨   │');
    console.log('│ ─────────────────────────       │ ─────────────────────────────       │');
    console.log('│ (no blockchain)                 │ prevHash (OUR ADDITION)             │');
    console.log('│ (no blockchain)                 │ opHash (OUR ADDITION)               │');
    console.log('│ (no blockchain)                 │ chainHash (OUR ADDITION)            │');
    console.log('│ (no sequence)                   │ seq (OUR ADDITION)                  │');
    console.log('│ (no node tracking)              │ origin (OUR ADDITION)               │');
    console.log('│ (no state tracking)             │ prevStateHash (computed) ✅         │');
    console.log('│ (no state tracking)             │ currentStateHash (computed) ✅      │');
    console.log('└─────────────────────────────────┴─────────────────────────────────────┘');
    ...
    console.log('  ✓ Bandwidth: Only send blobId, receiver checks "do I have this?"');
    ...
    console.log('\n\n📡 PART 5: Node-to-Node Synchronization\n');
    console.log('─'.repeat(80));
    console.log('Demonstrating actual sync between Node A (producer) and Node B (consumer)\n');
    ...
    const metaFromA = await db.collection('sync_state').findOne({ _id: 'sync_ops_meta' } as any);
    ...
    const componentsTable = JSON.parse(componentsBlob.content.toString('utf-8'));
    console.log(`✓ Fetched ComponentsTable with ${componentsTable._data.length} operations`);
    console.log(`  Payload size: ${(componentsBlob.content.length / 1024).toFixed(2)} KB`);
    ...
    for (const syncOp of componentsTable._data) {
      ...
      if (!docBlobId) {
        ...
        if (syncOp.operationType === 'delete') {
          ...
          if (typeof docIdToDelete === 'string' && /^[a-f0-9]{24}$/i.test(docIdToDelete)) {
            docIdToDelete = new ObjectId(docIdToDelete);
          }
          
          const deleteResult = await collectionB.deleteOne({ _id: docIdToDelete });
          console.log(`    ✓ Deleted from Node B database (deleted: ${deleteResult.deletedCount})`);
          opsApplied++;
      ...
      console.log(`    Original _id from blob: ${fullDocument._id} (type: ${typeof fullDocument._id})`);
      
      // Convert string _id back to ObjectId if it looks like an ObjectId
      if (typeof fullDocument._id === 'string' && /^[a-f0-9]{24}$/i.test(fullDocument._id)) {
      ...
      if (restoredDoc.name) {
        console.log(`    Document: ${restoredDoc.name} (${restoredDoc.role || 'N/A'})`);
      } else {
        console.log(`    Document: ${JSON.stringify(restoredDoc).substring(0, 60)}...`);
      }

      // Apply operation to Node B's database
      if (syncOp.operationType === 'insert' || syncOp.operationType === 'update' || syncOp.operationType === 'replace') {
        const result = await collectionB.replaceOne(
          { _id: restoredDoc._id },
          restoredDoc,
          { upsert: true }
        );
        console.log(`    ✓ Applied to Node B database (matched: ${result.matchedCount}, upserted: ${result.upsertedCount ? result.upsertedId : 'none'})`);
        opsApplied++;
      }
    ...
    for (let i = 0; i < operations.length; i++) {
      ...
      if (i === 0) {
        ...
        if (op.prevHash === 'GENESIS') {
          console.log(`  ✅ Op #${op.seq}: prevHash = GENESIS (chain starts correctly)`);
        } else {
          console.log(`  ❌ Op #${op.seq}: prevHash should be GENESIS, got ${op.prevHash}`);
          chainValid = false;
          errors++;
        }
      } else {
        ...
        if (op.seq === prevOp.seq + 1) {
          ...
          console.log(`  ❌ Op #${op.seq}: sequence gap detected (prev was ${prevOp.seq})`);
          chainValid = false;
          errors++;
        }
        ...
        if (op.prevHash === prevOp.chainHash) {
          console.log(`  ✅ Op #${op.seq}: prevHash links to Op #${prevOp.seq} chainHash`);
        } else {
    ...
    console.log();
    if (chainValid) {
      console.log(`  ✅ Blockchain chain verified! All ${operations.length} operations form valid chain`);
    ...
    if (countA === countB) {
      ...
      const stateA = await computeStateCheckpoint({
        db,
        ignoredColls: new Set(['sync_state', 'sync_local', 'sync_resume', 'sync_ops_received', 'state_checkpoints', 'state_merkle', 'state_dirty']),
        partitionSize: 50000,
        mode: 'full'
      });
      ...
      if (docsMatch && hashesMatch) {
        ...
        console.log('    1. Blockchain chain integrity: ✅ Verified (8 operations)');
        ...
        console.log('    • Blockchain: Operations weren\'t tampered with or reordered');
        console.log('    • State hash: Final database state is identical (O(1) check)');
        ...
        for (const doc of docsA) {
          ...
          if (doc.skills && doc.skills.length > 0) {
            console.log(`      Skills: ${doc.skills.slice(0, 3).join(', ')}${doc.skills.length > 3 ? '...' : ''}`);
          }
        }
        console.log();
        ...
        console.log(`    Node A has ${docsA.length} docs, Node B has ${docsB.length} docs\n`);
        
        if (docsA.length > 0 && docsB.length > 0) {
          console.log('    Sample Document A[0]:');
          console.log(`      _id: ${docsA[0]._id} (type: ${typeof docsA[0]._id})`);
          console.log(`      name: ${docsA[0].name}`);
          console.log();
          console.log('    Sample Document B[0]:');
          console.log(`      _id: ${docsB[0]._id} (type: ${typeof docsB[0]._id})`);
          console.log(`      name: ${docsB[0].name}`);
          console.log();
        }
        
        for (let i = 0; i < Math.min(docsA.length, docsB.length); i++) {
          const allKeys = Array.from(new Set([...Object.keys(docsA[i]), ...Object.keys(docsB[i])]));
          const diffKeys = allKeys.filter(key => 
            JSON.stringify(docsA[i][key]) !== JSON.stringify(docsB[i][key])
          );
          if (diffKeys.length > 0) {
            console.log(`    Document ${i + 1} differs in: ${diffKeys.join(', ')}`);
          }
        }
      }
    } else {
    ...
    console.log('  ✓ Hash-based verification: Cryptographic proof of identical state (Merkle tree)');
    console.log('  ✓ Small sync ops (~500 bytes) stored separately from documents (10KB+)');
    console.log('  ✓ Blob de-duplication: Same document synced 100x = fetched 1x');
    console.log('  ✓ Bandwidth efficient: Only send blobId, check "do I have this?"');
    console.log('  ✓ Blockchain integrity: Every operation cryptographically linked');
    console.log('  ✓ State verification: Can verify database state at any point');
    console.log('  ✓ Resume capability: Can restart sync from any sequence number');
    ...
    console.log('    ❌ Old way: Download all docs, compare each field → Slow, bandwidth-heavy');
    console.log('    ✅ RLJSON: Compare single hash → Instant, cryptographic proof');
    console.log('  \n  Benefits:');
    console.log('    • O(1) verification time (regardless of database size)');
    console.log('    • Detects ANY difference (even single byte in millions of docs)');
    ...
    console.log('  \n  Why it matters:');
    console.log('    • Audit trail: Prove operation history hasn\'t been altered');
    ...
    console.log('─'.repeat(80));
    console.log('Demonstrating dirty partition tracking and incremental mode\n');
    ...
    console.log(`  ✓ Full hash computed: ${initialState.dbRoot.substring(0, 32)}...`);
    console.log(`  ⏱️  Time: ${fullDuration}ms`);
    console.log(`  💾 Cache: Stored in state_merkle collection\n`);

    // Check cache
    const cachedPartitions = await db.collection('state_merkle').countDocuments({ coll: 'users' });
    console.log(`  Cache status: ${cachedPartitions} partitions cached for 'users' collection\n`);
    ...
    if (dirtyStatus.full) {
      ...
      console.log(`  Dirty status: ${dirtyStatus.partitions.length} partition(s) marked dirty`);
      console.log(`  Dirty partitions: [${Array.from(dirtyStatus.partitions).join(', ')}]\n`);
    }
    ...
    console.log(`  ✓ Incremental hash: ${incrementalState.dbRoot.substring(0, 32)}...`);
    ...
    for (const syncOp of componentsTable._data) {
      ...
      if (syncOp.operationType === 'delete') {
        if (typeof syncOp.docId === 'string' && /^[a-f0-9]{24}$/i.test(syncOp.docId)) {
    ...
    console.log(`  Node A state hash: ${incrementalState.dbRoot.substring(0, 32)}...`);
    
    if (countC === 3) {
      console.log(`  ✅ Backfill successful! Node C has complete database state\n`);
    }
    ...
    if (fullDuration > 0 && incrementalDuration > 0) {
      const savings = (((fullDuration - incrementalDuration) / fullDuration) * 100).toFixed(0);
      console.log(`  Time saved:       ${savings}% (${fullDuration - incrementalDuration}ms)`);
    }
    console.log();
    ...
    console.log('  ✓ Real-time viable: Fast enough for continuous verification');
    ...
    console.log('\n' + '═'.repeat(80));
    console.log('  Complete! Full RLJSON Architecture with Optimization Demonstrated');
    console.log('  MongoDB → RLJSON → Node Sync → Cache → Incremental Hashing → Backfill');
    console.log('═'.repeat(80) + '\n');
