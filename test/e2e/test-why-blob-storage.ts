...
async function main() {
  ...
  try {
    ...
    console.log(
      '   • BsGridFS (MongoDB GridFS)  ← Still uses MongoDB, but differently!',
    );
    ...
    console.log(
      '   Sync to PostgreSQL, MySQL, SQLite without MongoDB driver\n',
    );
    ...
    console.log(
      "   Git doesn't store full files in refs, neither should we!\n",
    );
  } catch (error) {
    console.error('\n❌ Error:', error);
    process.exit(1);
