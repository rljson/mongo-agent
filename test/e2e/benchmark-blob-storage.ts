...
async function test() {
  ...
  console.log(
    `${colors.green}✓${colors.reset} Stored ${docs.length} blobs in ${formatTime(storeTime)}`,
  );
  ...
  console.log(
    `${colors.green}✓${colors.reset} Retrieved ${docs.length} blobs in ${formatTime(retrieveTime)}`,
  );
