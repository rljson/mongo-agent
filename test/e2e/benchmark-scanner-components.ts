...
async function benchmark() {
  ...
  console.log(
    `${colors.cyan}Testing with cd_articles (552k documents)${colors.reset}\n`,
  );
  ...
  try {
    ...
    console.log(
      `${colors.yellow}→${colors.reset} Document count: ${count.toLocaleString()}`,
    );

    if (count === 0) {
      console.log(
        `${colors.yellow}⚠${colors.reset} Collection is empty. Creating 10,000 test documents...\n`,
      );
      ...
      for (let i = 0; i < totalDocs; i += batchSize) {
        const docs = Array.from(
          { length: Math.min(batchSize, totalDocs - i) },
          (_, j) => ({
            _id: `doc${i + j}`,
            title: `Article ${i + j}`,
            ...
            metadata: {
              category: ['Technology', 'Science', 'Business', 'Arts'][
                (i + j) % 4
              ],
      ...
      console.log(
        `${colors.green}✓${colors.reset} Created ${newCount.toLocaleString()} test documents\n`,
      );
    }

    const finalCount = await collection.countDocuments();
    console.log(
      `${colors.yellow}→${colors.reset} Ready to benchmark with ${finalCount.toLocaleString()} documents\n`,
    );
    ...
    if (collNode) {
      ...
      console.log(
        `  _data rows: ${componentsTable._data.length.toLocaleString()}`,
      );
      console.log(`  JSON size: ${(jsonSize / 1024 / 1024).toFixed(2)} MB`);
      console.log(
        `  Bytes/document: ${(jsonSize / componentsTable._data.length).toFixed(0)}`,
      );
      ...
      console.log(
        `  Keys (${keys.length}): ${keys.slice(0, 10).join(', ')}${keys.length > 10 ? '...' : ''}`,
      );
    ...
    if (tableCfg) {
      ...
      for (let i = 0; i < Math.min(10, tableCfg.columns.length); i++) {
        ...
        console.log(
          `  ${(i + 1).toString().padStart(2)}. ${col.key.padEnd(20)} ${col.type.padEnd(10)} "${col.titleLong}"`,
        );
      }
    ...
    console.log(
      `Expected blobs: ${expectedBlobs} (ComponentsTable + TableCfg)`,
    );
    ...
    console.log('═'.repeat(60));
    console.log(
      `${colors.green}✓${colors.reset} Successfully scanned ${finalCount.toLocaleString()} documents`,
    );
    console.log(
      `${colors.green}✓${colors.reset} Performance: ${(finalCount / scanDuration).toFixed(0)} docs/sec`,
    );
    console.log(
      `${colors.green}✓${colors.reset} Tree structure: ${tree.trees.size} nodes (database + collection)`,
    );
    console.log(
      `${colors.green}✓${colors.reset} No per-document tree nodes (all in ComponentsTable)`,
    );
    console.log(
      `${colors.green}✓${colors.reset} ComponentsTable: ${componentsTable._data.length.toLocaleString()} rows, ${(jsonSize / 1024 / 1024).toFixed(2)} MB`,
    );
    console.log();
  } catch (error) {
