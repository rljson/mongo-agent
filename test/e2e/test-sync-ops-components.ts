...
import { startDbChangeStream, createSuppressor } from '../../src/watch-changes';
...
async function test() {
  ...
  try {
    ...
    console.log(`${colors.cyan}Test 1: Start Change Stream with ComponentsTable${colors.reset}`);
    ...
    console.log(`${colors.cyan}Test 2: Insert Documents and Check ComponentsTable${colors.reset}`);
    console.log('─'.repeat(60));
    ...
    console.log(`${colors.yellow}→${colors.reset} Waiting for change stream to process operations...`);
    ...
    console.log(`${colors.cyan}Test 3: Retrieve and Verify ComponentsTable${colors.reset}`);
    console.log('─'.repeat(60));
    ...
    if (meta && (meta as any).componentsBlobId) {
      ...
      if (blob) {
        ...
        if (table._tableCfg === (meta as any).tableCfgHash) {
          console.log(`${colors.green}✓${colors.reset} _tableCfg matches metadata`);
        }

        if (table._data.length === 3) {
          console.log(`${colors.green}✓${colors.reset} Correct number of operations: 3`);
        } else {
          console.log(`${colors.yellow}⚠${colors.reset} Expected 3 operations, got ${table._data.length}`);
        }
        ...
        for (let i = 0; i < Math.min(3, table._data.length); i++) {
          ...
          console.log(`  ${i + 1}. ${op.operationType} on ${op.ns?.coll} (seq: ${op.seq}, _hash: ${op._hash})`);
        }
        ...
        for (let i = 1; i < table._data.length; i++) {
          ...
          if (curr.prevHash !== prev.chainHash) {
            chainValid = false;
            console.log(`${colors.yellow}⚠${colors.reset} Chain broken at operation ${i}`);
            break;
          }
        }

        if (chainValid && table._data.length > 1) {
          console.log(`${colors.green}✓${colors.reset} Blockchain chain valid (prevHash → chainHash)`);
        }
      ...
      console.log(`${colors.yellow}⚠${colors.reset} No ComponentsTable to retrieve\n`);
    }
    ...
    const local = await db.collection('sync_local').findOne({ _id: 'local' } as any);
    ...
    if (local) {
      ...
      if ((local as any).seq === 3) {
        console.log(`${colors.green}✓${colors.reset} Correct sequence number: 3`);
      } else {
        console.log(`${colors.yellow}⚠${colors.reset} Expected seq=3, got ${(local as any).seq}`);
      }
      console.log();
    } else {
