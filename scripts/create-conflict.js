  ...
  await articles.updateOne(
    { _id: articleId },
    {
      $set: {
        title: 'Understanding Database Synchronization',
        content:
          'This article explains database sync concepts in depth with examples.',
    ...
    versions: [
      {
        documentId: articleId,
        data: await syncOps
          .findOne({ origin: 'nodeA', documentKey: { _id: articleId } })
          .then((op) => op.fullDocument),
      ...
      },
      {
        documentId: articleId,
        data: await syncOps
          .findOne({ origin: 'nodeB', documentKey: { _id: articleId } })
          .then((op) => op.fullDocument),
        timestamp: Date.now(),
        nodeId: 'nodeB',
