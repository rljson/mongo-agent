  ...
  try {
    ...
    for (const collName of collections) {
      try {
        ...
        console.log(`  ${collName.padEnd(20)} ${count.toLocaleString()} documents`);
      } catch (e) {
    ...
    const nodeStats = await syncOps.aggregate([
      {
        $group: {
          _id: '$origin',
          totalOps: { $sum: 1 },
    ...
    for (const op of recentOps) {
      console.log(`\n  ${op._id}`);
      console.log(`    Origin: ${op.origin} | Seq: ${op.seq} | Type: ${op.operationType}`);
    ...
    const conflicts = await syncOps.aggregate([
      {
        $group: {
          _id: '$docId',
          operations: {
    ...
    if (conflicts.length === 0) {
      ...
      for (const conflict of conflicts) {
        ...
        conflict.operations.forEach(op => {
          console.log(`      ${new Date(op.ts).toISOString()} | ${op.origin} | ${op.type}`);
        });
      }
    }
