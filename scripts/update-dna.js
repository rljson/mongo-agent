...
const filesToBeDeleted = [
  ...
  'doc/workflows/update-from-mongo-agent.md',
];

const templateRepo = 'https://github.com/rljson/mongo-agent.git';
const localRepoPath = path.resolve(__dirname, '../../mongo-agent');
...
function replaceMongoAgent() {
  ...
  for (const file of replaceFiles) {
    ...
    if (fs.existsSync(filePath)) {
      ...
      content = content.replace(/mongo-agent/g, projectName);
